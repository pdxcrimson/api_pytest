...
def test_post_ids_all_return_valid_schema(client: APIClient, post_id: int):
...
def test_filter_posts_by_user(client: APIClient, user_id: int):
...
def test_filter_returns_expected_count(client: APIClient, field: str, value: int, expected_count: int):
